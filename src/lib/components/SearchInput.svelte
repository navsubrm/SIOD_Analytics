<script lang="ts">
	let { query = $bindable(), placeholder = 'Search By Name' } = $props();

	function clearQuery(e: Event) {
		e.preventDefault();
		query = '';
	}
</script>
...
<span>
	<input type="text" name="search-value" {placeholder} autoComplete="on" bind:value={query} />
	<div class="buttons">
		{#if query == '' || !query}
...
<style>
	span {
		...
		width: 90%;
		max-width: 650px;
		border: solid 2px var(--_button-color, var(--blue));
		border-radius: 50em;
	...
	input[type='text']:focus-within {
		color: var(--_button-color, var(--blue));
	}
	...
	.buttons {
		...
		right: 5px;
		top: 4px;
		color: var(--_button-color, var(--gold));
	}
	...
	input[type='text']:focus-within ~ .buttons > *:hover {
		color: color-mix(in lab, var(--_button-color, var(--orange)), #000000 30%);
	}
</style>
